    ...
    while(hasMore)
    {
        ...
        if(state_ == ExpectHeaders)
        {
            ...
            if(crlf)
            {
                ...
                {
                    state_ = GotAll;//已经解析到最后一行，就是那个只有“\r\n”的空行
                    //printf("解析3\n");
                    hasMore = false;
                }
                //解析完一行就偏移到下一行，包括那个空行，完事也得偏移
...
{
    bool succeed = false;
    const char* start = begin;//常量指针，指向的内容不能改变
    ...
    if(space != end && _setMethod(start,space))
    {
        ...
        if(space != end)
        {
            ...
            start = space + 1;//解析版本
            succeed = end - start == 8 && equal(start,end-1,"HTTP/1.");//string m(start,end),字符串最后一个字符为*（end-1）
            if(succeed)
            {
                if(*(end - 1 ) == '1')
                    _setVersion(HTTP11);
